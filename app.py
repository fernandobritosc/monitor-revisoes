...
try:
    ...
    class version:
        VERSION = "20.0.0-MASTER"
        STATUS = "Central de Missões Unificada"
            ...
            with st.form("cad"):
                ...
                if st.form_submit_button("CRIAR CONTA"):
                    ...
                    if res_tk.data:
                        ...
                        st.success("Guerreiro Recrutado!")
                    else: st.error("Token Inválido")
    st.stop()

# --- CENTRAL DE MISSÕES (Escolha ou Criação de Concurso) ---
...
if 'concurso_ativo' not in st.session_state:
    ...
    col_sel, col_novo = st.columns([2, 1.2])
    
    with col_sel:
        st.subheader("🎯 Selecionar Missão")
        if not editais: st.info("Nenhum edital ativo.")
        else:
    ...
    with col_novo:
        ...
        with st.form("novo_concurso_inicial"):
            ...
            if st.form_submit_button("CRIAR E ENTRAR", use_container_width=True):
                # Blindagem contra APIError: envia materia e topicos iniciais
...
if selected == "Dashboard":
    ...
    st.title(f"📑 Gestão de Matérias: {concurso_ativo}")
    ...
    if st.button("📥 Gerar Snapshot JSON (Backup)"):
