...
if 'usuario_logado' not in st.session_state:
    ...
    with c2:
        ...
        tab1, tab2 = st.tabs(["Acessar", "Novo Guerreiro"])
        with tab1:
            if not users:
                ...
                with st.form("login_form"):
                    u = st.selectbox("Quem é você?", list(users.keys()))
            ...
            with st.form("cadastro"):
                ...
                if st.form_submit_button("CRIAR CONTA"):
                    ...
                    if res_tk.data:
                        ...
                        st.cache_data.clear()
                        st.success("Sucesso! Vá em Acessar.")
...
if selected == "Dashboard":
    ...
    st.title("📑 Editais")
    t1, t2 = st.tabs(["Novo", "Add Matéria"])
    with t1:
        with st.form("n"):
...
if usuario_atual == "Fernando Pinheiro" and st.sidebar.button("🎟️ Gerar Token"):
    tk = "SK-" + ''.join(secrets.choice(string.ascii_uppercase + string.digits) for _ in range(4))
    supabase.table("tokens_convite").insert({"codigo": tk}).execute()
    st.sidebar.code(tk)
