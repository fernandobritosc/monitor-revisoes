...
# ============================================================================
# Adicione este código ao seu app_PRONTO.py

# Cole este código após as funções existentes (depois da função excluir_concurso_completo)

# ============================================================================
# FUNÇÃO: LISTAR TEMPLATES PÚBLICOS
# ============================================================================
...
# ============================================================================
# FUNÇÃO: VISUALIZAR MATÉRIAS DE UM TEMPLATE
# ============================================================================
...
def clonar_template(supabase, concurso_origem, novo_concurso, novo_cargo, user_id, data_prova=None):
    ...
    try:
        # Verificar se já existe um concurso com esse nome para o usuário
        ...
        if check.data:
            return {
                'success': False,
                'message': f'Você já tem um concurso chamado "{novo_concurso}"!'
            }
        
        # Buscar todas as matérias do template
        ...
        # Incrementar contador no template original
        supabase.rpc('clonar_template_edital', {
            'p_concurso_origem': concurso_origem,
            'p_novo_concurso': novo_concurso,
            'p_novo_cargo': novo_cargo,
            'p_user_id': user_id,
            'p_data_prova': data_prova.strftime("%Y-%m-%d") if data_prova else None
        }).execute()
        
        return {
            'success': True,
            'message': f'✅ Template clonado! {clonados} matéria(s) adicionada(s).'
        }
        
    except Exception as e:
        return {
            'success': False,
            'message': f'❌ Erro ao clonar template: {str(e)}'
        }

# ============================================================================
# FUNÇÃO: TRANSFORMAR SEU EDITAL EM TEMPLATE
# ============================================================================
...
def transformar_em_template(supabase, concurso, user_id, nome_template, descricao):
    ...
    try:
        # Verificar se o edital pertence ao usuário
        check = supabase.table("editais_materias")\
            .select("id")\
        ...
        return {
            'success': True,
            'message': f'✅ Edital "{concurso}" agora é um template público!'
        }
...
# ============================================================================
# INTERFACE: TELA DE TEMPLATES (ADICIONAR COMO NOVA OPÇÃO NO MENU)
# ============================================================================

def pagina_templates(supabase, user_id):
    """Página de templates públicos"""
    
    st.title("📚 Templates de Editais")
    
    st.markdown("""
    <div style='background: rgba(139, 92, 246, 0.1); padding: 1rem; border-radius: 8px; margin-bottom: 1rem;'>
        <h4>💡 O que são Templates?</h4>
        <p>Templates são editais pré-configurados que você pode clonar para começar rapidamente!</p>
        <ul>
        ...
        if not templates:
            ...
            for concurso, info in templates.items():
                with st.expander(f"📚 {info['nome']} ({info['clones']} clone(s))"):
                    ...
                    # Formulário para clonar
                    with st.form(f"form_clonar_{concurso}"):
...
1. Cole todo este código no seu app_PRONTO.py (após as funções existentes)

2. Adicione nova opção no menu principal:

   No seu option_menu, adicione:
   
   selected = option_menu(
       menu_title=None,
       options=["🏠 Home", "📚 Templates", "📝 Registros", "📊 Dashboard", ...],
       #                   ↑ ADICIONAR AQUI
       ...
   )

3. Adicione o if para a nova página:

   if selected == "📚 Templates":
       pagina_templates(supabase, user_id)

4. Execute migration_templates.sql no Supabase ANTES de testar!
