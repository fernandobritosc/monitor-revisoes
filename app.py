...
def fatiar_edital_bisturi(texto):
    ...
    for linha in linhas:
        ...
        # Detecta Tópico (Deve começar com número)
...
if st.session_state.missao_ativa is None:
    ...
    with tabs[1]:
        st.subheader("🤖 Importador")
        c1, c2 = st.columns(2)
        n_n, n_c = c1.text_input("Concurso"), c2.text_input("Cargo")
        pdf = st.file_uploader("Upload PDF", type="pdf")
        if st.button("🚀 EXTRAIR") and pdf and n_n:
            doc = fitz.open(stream=pdf.read(), filetype="pdf")
            st.session_state.temp_ia = fatiar_edital_bisturi("\n".join([p.get_text() for p in doc]))
