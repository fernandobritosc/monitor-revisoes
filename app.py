...
import re
import os
import fitz  # PyMuPDF: O motor mais estável para nuvem
...
def calcular_pendencias(df):
    ...
    pendencias = []
    # Garante existência das colunas de revisão
    ...
    for _, row in df.iterrows():
        ...
        # Lógica Blindada: Verifica pendências por janela de tempo
...
if st.session_state.missao_ativa is None:
    ...
    with tabs[1]:
        ...
        pdf = st.file_uploader("Arraste o Anexo Programático", type="pdf")
        
        if st.button("🚀 INICIAR ANÁLISE") and pdf and novo_n:
            with st.spinner("Mapeando matérias..."):
        ...
        if "temp_ia" in st.session_state:
            ...
            st.success(f"{len(res)} matérias mapeadas.")
